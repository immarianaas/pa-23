...
def testHundredAndTwo():
    res = interpreter.interpretMethod("dtu/compute/exec/Simple", "hundredAndTwo", [])
    assert res == 102


def testIdentity(a):
    res = interpreter.interpretMethod("dtu/compute/exec/Simple", "identity", [a])
    assert res == a
...
def testFactorial(n):
    res = interpreter.interpretMethod("dtu/compute/exec/Simple", "factorial", [n])
...
def testHelloWorld():
    res = interpreter.interpretMethod("dtu/compute/exec/Calls", "helloWorld", [])
...
def testFirstSafe(n):
    try:
        res = interpreter.interpretMethod("dtu/compute/exec/Array", "firstSafe", [n])
...
def testAccess(i, n):
    res = interpreter.interpretMethod("dtu/compute/exec/Array", "access", [i, n])
    assert res == n[i]


def testNewArray():
    res = interpreter.interpretMethod("dtu/compute/exec/Array", "newArray", [])
    assert res == 1
...
def testBubbleSort(arr):
    temp = arr.copy()
    interpreter.interpretMethod("dtu/compute/exec/Array", "bubbleSort", [arr])
