...
def complete_dict_short(dict):
    ...
    for k, v in dict.items():
        new_dict[get_small(k)] = list(
            {
                get_small(elem)
                for elem in v
                if elem not in primitiva_java_types
                and get_small(elem) != "*"
                and get_small(elem) != get_small(k)
            }
        )
...
def add_dependencies(dep_dict, full_dict):
    ...
    for k, v in full_dict.items():
        ...
        deps = set(dep_dict[k])
        all_others = set(
            v["composition"] + v["realization"] + v["inheritance"] + v["aggregation"]
        )
